...
| 7. 지도 컨트롤 | 적용 | 지도 버튼 셋·층 선택기. 나머지는 조사에서 "옮기면 안 됨"으로 판정났다. 지도 하단을 다시 짜며 나온 후보 넷도 앱에 남겼다 — 아래 "단계 7 안쪽" |
...
| 결과·후보 목록의 머리 줄과 정렬 | 적용 (`RoutexResultList`·`RoutexSortMenu`). 건물 행은 목록 **밖**이다 — N은 매장 수다 |
| 고른 값이 쌓이는 줄, 분류 둘러보기 줄 | 미착수 — 아래 7.4 참고 |

> **계약을 화면보다 먼저 설계했다가 세 개를 걷었다.** v0.2.8에서 칩 줄에 `dismiss`(선택 해제 ×)와
...
>
> **일곱 번째도 폰에서 나왔다.** 표의 줄 사이 선이 목록용 들여쓰기(leading 아이콘 열 64를 비우는
> `RoutexDividerRole.row`)를 쓰고 있었다. 표에는 비울 열이 없어서 선이 아무것도 아닌 자리에서
> 시작해 떠 보였다. `tableRow` 역할을 더해 고쳤다(v0.2.17).
>
> **화면에 안 뜨는 섹션은 옮기지 않는다.** 특징·위치 안내·공지는 계약에는 있는데 데모 데이터가
> 하나도 쓰지 않아 폰에서 열 방법이 없었다. 눈대중으로 옮기는 대신 **데모 데이터를 채우고 로컬
> 백엔드로 띄워서** 실제 화면을 만든 다음 옮겼다. 앞의 실패 넷이 전부 "화면을 보기 전에 정했다"였다.
>
...
| 상세 본문 탭(홈·메뉴·사진) | 적용 (`RoutexTabs`) |
| 소개 | 적용 (`RoutexTypography.body`). 문단 줄 고르기는 앱이 갖는다 |
| 특징·메뉴 배지 | 적용 (`RoutexBadge`). 분류 배지 색은 앱이 짝으로 넘긴다 |
| 위치 안내 | 적용 (`RoutexKeyValueRows`). 라벨을 아이콘으로 줄일 수 없는 값들이다 |
...
Navigation이 지도 **하단**을 다시 짜면서(바닥 고정 탭 줄, 건물 안에서만 뜨는 이슈 다이어리 판)
Runtime Kit 후보가 넷 나왔다. **넷 다 앱에 남긴다.**

| 항목 | 판정 | 이유 |
|---|---|---|
| 바닥 고정 탭 줄 | 앱에 남긴다 | 쓰는 화면이 하나뿐이다. `RoutexTabs`는 콘텐츠를 바꾸는 가로 탭이라 대체가 아니다 |
| 하단 판의 껍데기(접힘·펼침·치움 스냅) | 앱에 남긴다 | 소비처가 하나다. 판이 무엇을 담는지가 아직 한 번밖에 안 나왔다 |
| 시트 끌어 닫기(문턱·한 번만 닫기) | 앱에 남긴다 | `RoutexBottomSheet`이 **드래그를 소유하지 않기로** 이미 정해 뒀다. 앱 안에서 시트 둘이 한 파일을 함께 쓰는 것으로 족하다 |
| 제목 세리프(Playfair Display) | 앱에 남긴다 | 글꼴 가족을 늘리는 foundation 변경인데 쓰는 자리가 제목 한 줄이다 |

재검토 조건은 하나다 — **두 번째 소비처가 생기면.** 그 전에 옮기면 단계 4에서 세 번 걷어낸 것과
같은 일이 된다(화면보다 계약을 먼저 설계했다).

> **한 가지는 계약의 빈자리를 확인해 줬다.** 표면과 손잡이는 Kit이 갖는데 끄는 감각은 앱이 가져서,
> 앱의 시트 둘이 각자 다른 값으로 어긋나 있었다(하나는 아예 안 닫혔다). 지금은 앱 안에서 한 곳으로
> 모았다. 이 분업 자체가 문제였는지는 **두 번째 소비 앱이 생겨야 알 수 있다** — 그때까지는 Kit이
> 드래그를 소유하지 않는다는 현재 결정을 유지한다.
