...
- 경로 계획·안내 진행·도착 표면은 배경을 화면 하단까지 잇고 안전 영역은 표면
  내부에서 처리해, 홈 인디케이터 위에 카드처럼 떠 보이지 않게 했다.
...
- 장소 상세의 닫기는 이름 줄 끝에, 공유·저장은 출발·도착 줄 오른쪽에 배치해
  별도 상단 동작 줄과 중복 여백을 만들지 않는다.
- 장소 상세의 오른쪽 quiet action은 48dp 터치 영역을 그대로 두고
  보이는 glyph만 보정해, 제목·출발 버튼과 좌우 끝선이 다르게 보이지 않게 했다.
- `RoutexRecentList`가 최근 목록의 history 아이콘, 전체 삭제, 48dp 행과 좌우
  정렬선을 소유한다. 한 줄 `RoutexListCell`은 보조 동작 바깥에 세로 여백을
  중복으로 더하지 않는다.
- `RoutexRoutePlanner`는 편집 중인 위치 행 안에 입력 필드를 렌더링한다.
  소비 앱이 플래너 아래에 별도 검색 바를 하나 더 만들 필요가 없다.
- 도착 위치 행과 이동수단 사이 간격을 4dp로 줄여 같은 경로 계획 묶음으로 읽히게 했다.
...
- 포인트 색을 파랑에서 초록(`#0F5A46`)으로 바꿨다. `actionPrimary`·`actionPrimaryPressed`·
  `actionPrimarySubtle`·`accentBrand`·`focusRing` 다섯 역할이 새 teal 계단을 가리킨다.
  값만 바뀌고 역할·이름·대비 계약은 그대로다 — `accentBrand`는 여전히 3:1은 넘고 4.5는
  못 넘는 자리(흰 배경 4.12)에 있어 글자에 쓸 수 없다.
  **앱 영향: 코드는 안 바뀌고 화면은 바뀐다.** 브리지가 `RoutexColorTokens`를 통째로 얹으므로
  이 ref를 올리는 순간 **이미 옮긴 화면 전부가 초록으로 뒤집힌다.** 아직 안 옮긴 화면은 앱
  `primary`(하늘)로 남는다 — 지금까지 그 차이는 같은 계열의 명도 차이였지만 이제 색상 차이다.
  단계 8 전환 게이트의 `primary` 항목도 그만큼 넓어진다. **화면 단위로 검토한 뒤에 ref를
  올린다.**
- `statusInfo`는 파랑에 남겼다. 안내는 브랜드가 아니라 상태이고, 상태 넷이 서로 다른 색상을
  지켜야 색만으로 구분된다. `statusSuccess`의 초록(141°)과 포인트 초록(163°)도 색상을
  벌려 두었다.
- **지도 시각 토큰(`routeLine`·`routeCasing`·`location`·`destination`)은 파랑·빨강 그대로 둔다.
  미룬 것이 아니라 그렇게 정한 것이다.** 지도 위의 색은 브랜드가 아니라 데이터이고, 파란
  경로선과 파란 위치 점은 사용자가 이미 학습한 기호다. 둘 다 초록이 되면 경로선이 UI에 묻히고,
  실제 지도의 공원 초록과도 싸운다. 초록은 "앱이 주는 조작", 파랑은 "지도가 주는 데이터"로
  가른다 — 이 층을 따로 둔 이유가 이번에 실제로 쓰였다.
...
- **파괴적:** `RoutexFloorOption.id`가 `int` → `String`이다. Kit의 다른 옵션 타입은 전부 `String id`이고
  이것만 예외였는데, 층에는 안정적인 정수 식별자가 없다 — 소비 앱의 층 목록은 `B2`·`1F` 같은
  문자열이라 어댑터가 목록 순서에서 번호를 지어내야 했고, 층이 하나 추가되는 날 그 번호가 조용히
  밀린다.
- `RoutexFloorSelector`가 **한 번에 다섯 칸까지만 세우고 나머지는 굴린다.** 전량을 Column으로
  세우고 있어서 지하 4층·지상 8층인 건물이면 기둥 하나가 지도 좌측을 통째로 가렸다. 지금 층은
  뷰포트 가운데로 맞추고, 끝에서는 목록 밖으로 넘기지 않는다. 상한을 입력으로 열지 않은 이유는
  그 값이 화면별 취향이 아니라 "지도가 보여야 한다"는 이 시스템의 규칙이기 때문이고, 스크롤을
  소비 앱에 넘기지 않은 이유는 셀 높이가 비공개 값인데 스크롤 계산이 그 값을 알아야 해서다.
  **앱 영향:** 다섯 이하이면 이전과 같다.
...
- `RoutexMenuList.thumbnailAspectRatio` 추가. 썸네일을 늘 정사각으로 **채우고 있었다**(`cover`).
  매장 사진은 잘려도 무엇을 찍었는지 남지만 **제품 사진은 모양 자체가 정보다** — 컵 위아래가 잘려
  뭉툭해진다. 배경색으로 여백을 채우는 방법도 못 쓴다(사진마다 배경이 다르다). 비율을 아는 쪽은
  사진을 가진 소비 앱이라 값으로 받고, 한 목록의 사진은 같은 출처라 줄마다가 아니라 목록에 한 번
  받는다. **앱 영향:** 안 주면 이전과 같다.

## 0.2.17

- `RoutexDividerRole.tableRow` 추가하고 `RoutexKeyValueRows`가 그것을 쓰게 했다. 표의 줄 사이 선이
  `row` 역할을 쓰고 있었는데, 그 역할은 목록 행의 **leading 아이콘 열을 비우려고** 64를 들여쓴다.
  표에는 비울 열이 없어서 선이 아무것도 아닌 자리에서 시작해 떠 보였다. **소비 앱의 메뉴 팝업을
  폰에서 열다 눈에 띄었다.** `row` 자체는 그대로다. **앱 영향:** 표의 선만 표 폭으로 늘어난다.

## 0.2.16

- `RoutexInfoRow.onCopied` 추가. 복사 성공 알림을 소비 앱이 가져갈 수 있다. Android 13부터는
  **시스템이 복사 확인을 스스로 띄워서**, 그 위에 컴포넌트 토스트를 얹으면 `복사했습니다`가 두 번
  뜬다. 그 판정은 기기 SDK를 조회해야 알 수 있어 이 패키지가 할 수 없고, 아는 쪽은 소비 앱이다.
  실패 알림은 이 값과 무관하게 컴포넌트가 계속 맡는다 — 시스템 확인은 성공에만 뜬다.
  **앱 영향:** 안 주면 이전과 같다.
