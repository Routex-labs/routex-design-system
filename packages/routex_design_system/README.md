...
사용량이 검증된 핵심 컴포넌트다. v0.2에서 Navigation 앱의 매장 상세·목록·대중교통 화면이
쓰던 역할을 마저 들여와, 앱이 사설로 그리던 표면이 남지 않게 했다.

현재 proposal에는 색상, 타이포그래피, 간격, 곡률, 공통 metric, 지도 위 layer와 제품 motion
token이 들어 있다. 화면의 여백과 자식 간격을 semantic role로 제한하는 `RoutexInset`,
`RoutexStack`, `RoutexCluster`도 proposal로 제공한다. v0.1 핵심 컴포넌트 10종은 모두
beta이며 값과 API는 `v0.1.0` 전에 조정할 수 있으므로 아직 stable이 아니다.
...
| 장소 패턴 | `RoutexPlaceHeader`, `RoutexPlaceActions`, `RoutexHours`, `RoutexMenuList`, `RoutexLinkList`, `RoutexResultList` | 장소 정보 순서, 출발·도착 위계, 영업 상태와 요일 표, 판매 목록, 외부 채널, 목록의 세 상태 |
...
- **모든 컨트롤**의 시각 높이는 칩 32dp, 그 외 44dp이고 터치 영역만 padded tap target으로
  48dp를 유지한다. 보이는 크기를 48로 만들면 시트와 지도 오버레이가 그만큼 두꺼워진다.
- 아이콘은 `RoutexIcons`의 의미를 쓴다. 화면이 `Icons.*`에서 글리프를 직접 고르지 않는다.
  분류·시설처럼 데이터에서 오는 아이콘만 소비 앱이 소유한다.
- focus는 `RoutexFocusRing`의 링으로만 표시한다. 채움(배경색)은 selected·hover·pressed가
  나눠 쓰므로 focus까지 채우면 네 상태가 한 언어를 공유하게 된다.
...
- 상단 바 왼쪽 자리는 비우지 않는다. `RoutexSearchLeading.menu` 또는 `.back` 중 하나를
  반드시 넘긴다.
- 카드·패널·지도 chrome의 색·곡률·경계·깊이는 `RoutexSurface`의 역할(flat/outlined/onMap/
  chrome) 하나로 고른다. 컴포넌트마다 `Material`을 다시 조립하지 않는다.
